<template>
  <el-container class="familyHome" direction="vertical">
    <!-- header -->
    <nav-top></nav-top>
    <!-- 下边 -->
...
<script>
// @ is an alias to /src
import familySlider from '@/components/FamilySlider.vue'
...
export default {
    ...
    tabRemove (targetName) {
      ...
      if (this.activeIndex === targetName) {
        ...
        if (this.openTab && this.openTab.length >= 1) {
          ...
          this.$router.push({path: '/familyMain'})
        }
      }
    }
  },
  mounted () {
    ...
    if (this.$route.path !== '/familyMain' && this.$route.path !== '/familyHome') {
      console.log('1')
      this.$store.commit('add_tabs', {route: '/familyMain', name: '家属首页'})
      ...
      console.log('2')
      this.$store.commit('add_tabs', {route: '/familyMain', name: '家属首页'})
      this.$store.commit('set_active_index', '/familyMain')
      // this.$router.push('/')
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (!flag) {
        ...
        if (to.path === '/familyHome' && to.name === undefined) {
          return
        }
